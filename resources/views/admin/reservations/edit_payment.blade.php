...
<body>
    <div class="bg-gray-200 flex justify-center items-center h-screen">
        <div class="bg-white p-6 rounded-md md:w-1/3 w-full mx-4">
            <h2 class="text-center font-car font-medium text-xl">{{ $reservation->pakaianAdat->nama }}
                ({{ $reservation->pakaianAdat->jenis }})</h2>
            <h2 class="text-start mt-4 text-gray-500 ">Status Pembayaran Saat Ini: <span
                    class="text-lg text-gray-800">{{ $reservation->payment_status }}</span></h2>
            <div>
                <form action="{{ route('admin.updatePayment', ['reservation' => $reservation->id]) }}" method="POST">
                    @csrf
                    @method("PUT")
                    <div class="my-5 w-full flex items-center">
                        <label class="w-44" for="payment_status">Status Pembayaran: </label>
                        <select
                    ...
                    <div class="flex justify-center mt-12">
                        <a href="{{ route('admin.reservations.index') }}" class="p-3 w-full text-white text-center rounded-md me-2 bg-red-600 hover:bg-red-800" >Cancel</a>
