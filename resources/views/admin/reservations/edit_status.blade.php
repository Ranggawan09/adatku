...
<body>
    <div class="bg-gray-200 flex justify-center items-center h-screen">
        <div class="bg-white p-6 rounded-md md:w-1/3 w-full mx-4">
            ...
            <h2 class="text-start mt-4 text-gray-500 ">Status Sewa Saat Ini: <span
                    class="text-lg text-gray-800">{{ $reservation->status }}</span></h2>
            <div>
                <form action="{{ route('admin.updateStatus', ['reservation' => $reservation->id]) }}" method="POST">
                    @csrf
                    @method("PUT")
                    <div class="my-5 w-full flex items-center">
                        <label class="w-44" for="status">Status Sewa: </label>
                        <select
